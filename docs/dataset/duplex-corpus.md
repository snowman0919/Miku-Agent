...
Event sequence가 다르다는 사실만으로 effective unique로 계산하지 않는다. Normalized/lexical
중복과 revision-pinned multilingual semantic embedding 군집을 함께 보고하고, corpus 수량 gate에는
선언한 primary semantic threshold의 cluster count를 사용한다.
