...
| Korean foundation text의 acceptance tokenizer와 Wikimedia share-alike 준수 방식은 무엇인가 | 100M token gate와 downstream artifact 의무를 재현 가능하게 계산해야 한다 | model tokenizer 고정, corpus tokenizer 고정; attribution manifest와 derivative policy | token 안정성, PII/boilerplate filter, document/sentence near-dedup, CC BY-SA attribution/share-alike | 공식 dump의 작은 quarantine slice로 compliance·quality audit | V0.2.0 |
