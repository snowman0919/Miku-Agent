...
`unknown` 또는 `restricted` source/sample은 accepted training set에 들어갈 수 없다. 권리 근거가 불충분하면 quarantine을 유지한다. Raw media, processed audio, dataset, weight는 Git에 넣지 않고 V0.2.0에서 access-controlled object storage와 immutable registry 방향을 검증한다.
